 ...
 */
#ifndef LIBBITCOIN_CONSENSUS_SCRIPT_HPP
#define LIBBITCOIN_CONSENSUS_SCRIPT_HPP
...
{
public:
    BCX_API void join(const script_type& other);
    BCX_API void push_operation(const operation& oper);
    BCX_API bool run(
        script_type input_script, const transaction_type& parent_tx,
        uint32_t input_index, bool bip16_enabled=true);

    BCX_API payment_type type() const;

    BCX_API const operation_stack& operations() const;

    BCX_API static hash_digest generate_signature_hash(
...
BCX_API opcode data_to_opcode(const data_chunk& data);
BCX_API std::string opcode_to_string(opcode code);
BCX_API opcode string_to_opcode(const std::string& code_repr);
BCX_API std::string pretty(const script_type& script);
BCX_API script_type unpretty(const std::string& pretty);
BCX_API std::istream& operator>>(
    std::istream& stream, script_type& script);
BCX_API std::ostream& operator<<(
    std::ostream& stream, const script_type& script);

BCX_API script_type raw_data_script(data_slice raw_script);
BCX_API script_type parse_script(data_slice raw_script);
BCX_API data_chunk save_script(const script_type& script);
BCX_API size_t script_size(const script_type& script);

BCX_API bool check_signature(data_slice signature,
    const ec_point& public_key, const script_type& script_code,
    const transaction_type& parent_tx, uint32_t input_index);
BCX_API bool create_signature(data_chunk& signature,
